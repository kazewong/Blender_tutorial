...
"""
athena_read.py copied from athena++
"""
...
def ReadData(fname):
    my_MeshGen = GetMeshGen(nth_lo=16, nth_hi=16, dth_pole=0.3, h_hi=0.05, reflect=True)
    f = athdf(fname, face_func_2=my_MeshGen)
    d = {}
    for k in ['x1v','x3v','x1f','x3f']:
        d[k] = f[k]
    d['x2v'] = np.concatenate((f['x2v'],np.pi-f['x2v'][::-1]))
    d['x2f'] = np.concatenate((f['x2f'][:-1],np.pi-f['x2f'][::-1]))
    for k in ['rho','press','vel1','vel2','vel3']:
        s = -1 if k=='vel2' else 1
        d[k] = np.concatenate((f[k],s*f[k][:,::-1,:]),axis=-2)
    return d