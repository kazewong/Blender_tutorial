...
def from_rRAdec_to_XYZ(r, RA, dec):
    x = r * np.cos(dec * np.pi / 180.0) * np.sin(RA * np.pi / 180.0)
    y = r * np.cos(dec * np.pi / 180.0) * np.cos(RA * np.pi / 180.0)
    z = r * np.sin(dec * np.pi / 180.0)
    theta = 0.5 * np.pi - dec * np.pi / 180.0
    phi = RA * np.pi / 180.0
...
ra_g, dec_g, z_km_s_g, Magnitude_g = np.loadtxt(
    path_halos, skiprows=5, usecols=(3, 4, 5, 6), unpack=True
)
ra_g = ra_g[:-1]
dec_g = dec_g[:-1]
